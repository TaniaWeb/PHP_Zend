 ...
 */
    ...
    /**
     * Publisher the template results to HTML
     * @auth  li chang hyok
     * @param string $resource_name
     ...
     * @param boolean $display
     */
    function publishHtml($resource_name, $outpath, $cache_id = null, $compile_id = null, $display = false)
    {
        static $_cache_info = array();
		
		// << prepare for static html .......
		$path_parts = pathinfo($outpath);
		$dirpath =  $path_parts["dirname"];
		$pathsplit = split('/', $dirpath);
		$extention =  $path_parts["extension"];
		
		if($extention != "html" && $extention != "htm" && $extention !="rdf") return false;
   ...
   /**
    ...
    {
		//	{{	KHU - 20070905
		//		$resource_name is one-depthed-tpl name, so compiled file name of some tpl file
		//			was duplicated, that's why, in case of those tpl file we make Smarty compile
		//			them newly in every time.
		if( $resource_name == "estate/index.tpl" || 
			$resource_name == "index/index.tpl" || 
			$resource_name == "include/header1.tpl" || 
			$resource_name == "include/header2.tpl" || 
			$resource_name == "include/header.tpl" || 
			$resource_name == "include/footer.tpl" )
			return false;
		//	}}	KHU
...
//echo($resource_name.$_source_content.$_compiled_content.$_cache_include);
